...
> 把你和 AI 讨论过的目标、拆分、进度和完成证据留在项目里，不再随着对话结束而丢失。

你可以在当前 Codex、Claude Code 或其他 MCP Runtime 里直接说一个还很模糊的想法。Runtime 会继续和你对话，把它澄清成可执行的 Goal Tree；之后换一个 Session，仍能从 GoalBoard 找回项目候选、未完成工作、阻塞和验收依据。

Web 是可选的查看与确认界面，不是录入 Goal 的必经步骤。GoalBoard 不替你启动 AI，也不把任务强塞给某个 Runtime；当前 Runtime 从可做的 Goal 中自主选择并推进。

```text
你说一个想法 → 当前 Runtime 和你澄清 → 你确认 Goal Tree
→ Runtime 自主选择可做项 → 实现与验证 → GoalBoard 保存进度和完成依据
```
