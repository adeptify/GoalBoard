# @adeptify/goalboard-plugin-actions

<!-- Generated as the F2 contract-only workspace boundary. -->

Status: `contract-only`  
Workspace path: `plugins/native/actions`  
Contract entrypoint: `@adeptify/goalboard-contracts/platform/plugin`

## Purpose

First-party Actions navigation, UI, and Module composition boundary.

This package explicitly does **not** own Action facts, dispatch workers, or Provider protocols.
...
```bash
pnpm --filter @adeptify/goalboard-plugin-actions typecheck
pnpm --filter @adeptify/goalboard-plugin-actions build
```
