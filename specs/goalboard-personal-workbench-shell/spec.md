...
- Desktop 的“上下文 / 进展 / 关系 / 记录”内容面板默认占满可用宽度，并为当前选中内容提供足够的视口高度；短内容使用安静的工作面承接剩余空间，不在面板下方留下大片无归属留白。760px 以下不强制最小高度；
...
11. Desktop 的上下文、进展、关系和记录标签内容默认撑满可用宽度，并以响应式最小高度承接当前视口；短内容不再在卡片下方留下大片空白，窄屏不被强制拉高。
