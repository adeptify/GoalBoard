...
Before closing any complex parent, call `goalboard_v1_planning_methods`, recover the original user outcome, and inspect the current task's work type, professional domains, deliverables, operating context, uncertainty, and risks. Select methods from this task inspection without preferring a method kind, library order, or fixed count. If `composition.method_pack_ids` is non-empty, use every method in that project composition as a required floor and record every ID; then add every other available method whose distinct checks materially apply. If the composition is empty, select all methods justified by the task, including multiple work types or domains when needed. Before proposing the tree, compare the selected coverage with the available method summaries and coverage rules. If an unselected method contributes a material check that remains uncovered, the Goal setup is incomplete and that method must be included. Keep selected paths as complementary reasoning passes, then use the merged coverage, dependency, evidence, completion, and failure rules on the same Goal Tree. When no domain fits, use `meta-domain-pack-builder` to establish a user-confirmed domain method before decomposing the actual work. Every new proposal still accounts for `final_outcome`, `operating_flow`, `core_capabilities`, `foundation_infrastructure`, and `quality_continuous_delivery`. For each required area, use exactly one coverage entry:
...
When the user adds or changes a requirement, call `goalboard_v1_planning_analyze_change` with the directly affected Goal IDs before proposing edits. Preserve unaffected Goals; review returned ancestors and downstream consumers in order, reuse compatible open Goals, then submit only the changed contracts and relations. This is impact analysis, not permission to rewrite the tree automatically.
