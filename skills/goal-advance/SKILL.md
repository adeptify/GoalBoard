...
| A new requirement changes existing work | Read the named or affected Goals, then call `goalboard_v1_planning_analyze_change`. Reuse the unaffected tree, review the returned ancestors and dependents in order, and submit only the necessary Goal Tree changes for user confirmation. |
...
Before decomposing a complex Goal, call `goalboard_v1_planning_methods`, then inspect the current task's intended outcome, kind of work, professional domains, deliverables, operating context, uncertainty, and risks. Decide which methods materially apply from that inspection, not from template order, method kind, or a target count. When `composition.method_pack_ids` is non-empty, treat the entire project composition as a required floor and record every ID in `decomposition_review.method_pack_ids`; then add every other available method whose distinct checks materially apply to this Goal. When the composition is empty, select all methods justified by the current task; the result may cross several work types or domains and may contain one or many methods. Compare the selected methods' coverage against the available methods before proposing the tree: if an unselected method contributes a material check that is still uncovered, include it. Omitting a materially relevant method makes the Goal setup incomplete. Preserve each selected method's own path instead of concatenating all steps into a serial task list, then merge their coverage, dependency, evidence, completion, and failure checks across the same Goal Tree. Do not copy one method label into one Goal mechanically.

If no domain method fits, select `meta-domain-pack-builder` first. Research the domain objects, lifecycle, professional artifacts, evidence standards, dependency patterns, and failure modes. Keep uncertain conclusions explicit. After the user confirms the resulting method, save it with `goalboard_v1_planning_method_save`; only then use it to decompose the actual Goal. Do not pretend an adjacent domain template is good enough.

Before saying any complex Goal is fully decomposed, return to the user's original outcome rather than the topic discussed most recently. The built-in build/change method checks the same five-part result chain for every task: the final result and who uses it; the real operating or usage flow; the core capabilities that do the main work; the foundations and infrastructure those capabilities need; and quality, delivery, recovery, and continued operation.

Then apply every task-specific omission check that materially fits the current Goal. A game also covers gameplay, game systems/content, player journey, interaction/UI, and audiovisual experience. An App covers its core function, end-to-end journey, interaction/UI, and content/information. AI or data work covers data sources and quality, evaluation, runtime/cost, and safety/governance. Content or research covers source provenance, method or production flow, review/approval, and publication/distribution. Operations covers roles, permissions, tools/workflow, exceptions, and measurement. A Goal spanning several of these areas must cover each relevant check; `other` still completes the five-part chain instead of treating a non-software task as having no foundation.
...
For every `depends_on`, state which usable provider output the consumer needs. Run `goalboard_v1_planning_graph_check` after confirmed changes. When choosing ordinary Available work, use the returned `planning` rationale together with user intent and blockers; it explains order but never dispatches a mandatory task.
