...
- **Focus:** Blue boundary and restrained blue focus ring; the single toolbar field filters the visible Tree immediately.
...
- **Entry:** The toolbar funnel icon is an explicit disclosure trigger with `aria-expanded` and `aria-controls`; it opens the panel below the trigger and focuses the first status instead of silently changing the search-field focus.
...
- **Do** keep the Draft-only editor with acceptance, scope, inputs/outputs, and relations in the completion chapter; keep accepted Contracts read-only in place.
