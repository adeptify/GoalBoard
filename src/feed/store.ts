...
export function migrateFeedTables(db: Database.Database): void {
  db.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS feed_items_board_external_idx
...
export class FeedStore {
  ...
  snapshot(boardId: string): FeedSnapshot {
    ...
    ).all(boardId) as Row[]).map(mapFeedSource);
    ...
    ).all(boardId) as Row[]).map((row) => mapFeedItem(row, materialsByItem.get(text(row.item_id)) ?? []));
    ...
    return { sources, items, runs, import_receipts: importReceipts };
  }
  ...
  findSource(
    ...
    return rows.map(mapFeedSource).find((source) => {
  ...
  ingestItem(input: {
    ...
    const existing = this.db.prepare(`
      ...
      WHERE board_id = ? AND source_kind = ? AND external_id = ?
    `).get(input.source.board_id, input.source.kind, input.externalId) as { item_id: string } | undefined;
    if (existing) return { item: this.getItem(input.source.board_id, existing.item_id), created: false };
    const now = new Date().toISOString();
    ...
    `).run(
      ...
      input.source.sync_kind === "public_source" ? "feed" : "inbox_message",
      input.kind ?? "update",
