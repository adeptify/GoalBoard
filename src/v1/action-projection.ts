...
interface ActionProjectionIndex {
...
function projectionIndex(snapshot: BoardSnapshot): ActionProjectionIndex {
...
function compareActions(left: GoalAction, right: GoalAction): number {
  const priority = (item: GoalAction): number => {
    // A pending contract decision or Human Review is allowed to interrupt
    // Runtime work. Other active Claim/Run actions remain ahead of ordinary
    // review, evidence and Risk work.
...
function currentEvidence(goal: GoalRecord, snapshot: BoardSnapshot) {
  const compatibleRevisions = compatibleContractRevisions(goal, snapshot);
  return (projectionIndex(snapshot).evidence_by_goal.get(goal.goal_id) ?? []).filter((evidence) =>
...
function criterionPassed(goal: GoalRecord, snapshot: BoardSnapshot, criterionId: string): boolean {
  ...
  const evidence = currentEvidence(goal, snapshot)
    .filter((item) => item.criterion_ids.includes(criterionId))
